<?php
	...
	if (($pw == 'supergeil') && ($msg != null)) {
		...
		{
			$msg = str_replace('\\"','"',$msg);
			$returnValue = preg_match_all('/<tr class="(?:\s+|\s+ally\s+)">\s+<th>([0-9]+)\s+(?:<span class="[a-z]+"><small>&[a-z]+;[0-9]+<\/small><\/span>|&bull;)\s+<\/th>\s+<th>\s+<a href="playercard\.php\?u=[a-zA-Z0-9\-_\.]+">([a-zA-Z0-9_\-\.]+)<\/a>\s+(?:&nbsp;\[<a href="allianzen\.php\?ally=[a-zA-Z0-9\-_\.]+">([a-zA-Z0-9\-_\.]+)<\/a>\])?<\/th>\s+<th>([0-9\.]+)<\/th>\s+<th>([0-9\.]+)<\/th>\s+<th>([0-9\.]+)<\/th>\s+<th>([0-9\.]+)<\/th>/',$msg,$ausgabe);
		}
		...
		foreach($ausgabe[0] as $i => $value)
		{
			...
			$q = "INSERT INTO highscore(hs_platz,hs_name,hs_allianz,hs_planetenpunkte,hs_forschungspunkte,hs_punkte,hs_planeten) VALUES".
			"(".
				$ausgabe[1][$i].",".
				"'".$ausgabe[2][$i]."',".
				"'".$ausgabe[3][$i]."',".
				str_replace(".", "", $ausgabe[4][$i]).",".
